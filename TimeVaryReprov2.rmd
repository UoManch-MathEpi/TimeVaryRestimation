...
This file creates the figures from Pang *et al* 2024 [Pang](https://www.medrxiv.org/content/10.1101/2024.03.26.24304928v1) (there will be slight differences as different random seeds are used). 
...
out=data.frame(read.csv('Carehome_Outbreaks.csv'))

sis <- data.frame(t=out$t, x=out$x)
```
...
Figure numbers in section titles below refer to figures in Pang et al, 2024. They do not reflect the order they appear in paper.
...
```{r sis, warning=FALSE}
PredictGAMSIS <- gamTVRpred(data=sis, type='SIS', para=c(2500, 14500, 26,0), EEwindow = 0, wrtfile = TRUE)
```
...
outR2 <- SimulatorTVRSIS(epipara = c(10000, 25, 2, 100, 2), plotgraph=TRUE)
...
p1 <- ggplot(data=SummaryOutput)+
  ...
  geom_vline(xintercept=as.Date(18353+365-20), color='white', lwd=2)+
  geom_segment(x=as.Date(SummaryOutput$dates[1]), xend =as.Date(18353+365), y=2, colour='white', lwd=2)+
...
```{r pcr, warning=FALSE}
pcrcut <- pcr[-(1:10),]
PredictGAM <- gamTVRpred(data=pcrcut, para=c(500, 450000, 5, 3), type='SEIR', wrtfile = TRUE)
```
(we drop the first few observations as the numbers are very small and likely unreliable due to testing being limited due to PCR capacity, this is likely true to varying extents until Sept 2020 when asymptomatic testing (and LFD) was introduced).

## Figure 2: Simulations

Here we simulate 12 different scenarios with different reproduction numbers, dates of intervention and population size, which will produce plots like Figure 2 and panels a, b and c in Figure 4.
...
p1 <- ggplot(data=SummaryOutput)+
  ...
  geom_segment(x=as.Date(SummaryOutput$dates[1]), xend =as.Date(18353+121), y=R0[3], colour='black')+
  geom_segment(xend=as.Date(18353+121+90), x =as.Date(18353+121), y=R0[3]/2, colour='black') +
  geom_segment(x=as.Date(SummaryOutput$dates[1]), xend =as.Date(18353+121), y=R0[2], colour='red')+
  geom_segment(xend=as.Date(18353+121+90), x =as.Date(18353+121), y=R0[2]/2, colour='red')+
  geom_segment(x=as.Date(SummaryOutput$dates[1]), xend =as.Date(18353+121), y=R0[1], colour='blue')+
...
Then consider a 4th plot of inference over the intervention.
...
enddate <-  as.Date(offset+duration)
  ...
  for(i in c(seq(0,16,2),20,24, floor(dim(Sim)[1]/repTS)-120)){  
    interventdate <- 120
     tvrout <- gamTVRpred(data=Simdata[1:(interventdate+i),], para=c(Simdata[1,2]*InfP,pop, InfP,IncP), 
...
Here we show how an the central estimator can be derived from the model.

Firstly simulate some data assuming a disease, COVID-like in terms of duration but with R0= 1.39 and with 10% case fatality ratio (assuming people die at end of infectious period).

```{r}
incubation <- 3
infecitous <- 5
SimDat <- seir_tau_tv(MaxTime=10*400,dt=1/10, repro=2*log(2),sig=1/incubation, gam= 1/infecitous,
                      N0=10^5, I0=10,E0=0, itp=1, frac=1, plt1 = T, plt2 = T )
SimDeath <- rbinom(400, prob=0.1, size=diff(SimDat$R[seq(1,4001, 10)]))
PredMort <- gamTVRpred(cbind(18350+1:400, SimDeath), plt=FALSE)
```

We now rather than use the internal plotting functions extract the derivatives and estimator of effective reproduction number 
```{r}
PredMort$REdeath <- 1+infecitous*incubation*PredMort$sdt2+infecitous*incubation*PredMort$sdt^2+
  (incubation+infecitous)*PredMort$sdt

    p1 <- ggplot(data=PredMort, aes(x=time, y=sdt))+
      geom_line(color='darkblue') +
      labs(x='Date',y='R_E', title='First derivative of spline')
    p2 <- ggplot(data=PredMort, aes(x=time, y=sdt2))+
      geom_line(color='darkblue') +
      labs(x='Date',y='R_E', title='Second derivative of spline')
    p3 <- ggplot(data=PredMort, aes(x=time, y=sdt3))+
      geom_line(color='darkblue') +
      labs(x='Date',y='R_E', title='Third derivative of spline')
    p4 <- ggplot(data=PredMort, aes(x=time, y=REdeath))+
      geom_line(color='darkblue') +
      labs(x='Date',y='R_E', title='R_E estimate from death data')

      print(p1+p2+p3+p4+plot_layout(ncol=2))

```

Derivatives show the expected behaviour of a cubic spline (i.e. piecewise constant at third derivative, piecewise linear for second) and effective R estimate starts at the theoretical value and drops off to 50% (which we would expect).


```{r pcrsim, eval=TRUE, warning=FALSE}
R0 <- c(rep(c(log(4/3)*4, log(4)*4/3, 2*log(2)),1))
SummaryOutput <- data.frame(dates = seq(18353, 18353+365,1)) 
Inttime <- c(rep(1, 3))
pop <- c(rep(10^5,3))
for(inc in seq(3,1,-1)){
    out <- SimulatorTVRSEIR(epipara = c(pop[inc], R0[inc], 5, 3, 10, 1), contpara=c(Inttime[inc],0.5), plotgraph = TRUE)
    out <- data.frame(out)
    colnames(out) <- c('dates', paste('RE', inc,sep=''), paste('REup', inc,sep=''), paste('RElow', inc,sep=''), 
paste('RC', inc,sep=''), paste('RCup', inc,sep=''), paste('RClow', inc,sep=''))
    SummaryOutput <- Reduce(function(x, y) merge(x, y, all=TRUE), list(SummaryOutput, out))
